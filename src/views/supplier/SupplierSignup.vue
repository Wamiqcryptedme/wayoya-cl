...
<input type="text" v-model="formData.ntn" class="w-full text-sm px-4 py-3 border-[1.5px] border-gray-200 rounded-xl outline-none focus:border-brand-teal">
    ...
    <div v-if="currentStep === 5" class="animate-fade-in space-y-6">
      ...
      <div v-if="formData.businessType === 'Company'">
        ...
        <div v-else class="border border-gray-200 rounded-xl p-4 flex justify-between items-center">
          ...
          <button @click="removeDocument('companyReg')" class="text-gray-400 hover:text-red-500">✕</button>
        </div>
      </div>
      ...
      <div>
        ...
        <div v-else class="border border-gray-200 rounded-xl p-4 flex justify-between items-center">
          ...
          <button @click="removeDocument('license')" class="text-gray-400 hover:text-red-500">✕</button>
        </div>
    ...
    <div v-if="currentStep > 1" class="mt-6">
      <button 
        ...
        :disabled="isLoading"
        class="w-full py-3.5 bg-brand-teal hover:bg-brand-tealDark text-white font-bold rounded-xl shadow-lg shadow-teal-500/30 transition-colors relative flex items-center justify-center disabled:opacity-50">
    ...
    <!-- Error Message -->
    <div v-if="errorMessage" class="mt-4 p-3 bg-red-50 border border-red-100 rounded-xl text-red-500 text-sm text-center font-medium flex items-center justify-center gap-2 animate-shake">
...
if (documentFiles.license) {
  const licenseUrl = await uploadService.uploadDocument(
    ...
    'temp', // Will be replaced with actual user ID after signup
    'license'
  );
...
// Step 2: Upload Property Photos (if Stay)
...
// Step 3: Sign up user (triggers verification email)
await authService.signUp(formData.email, formData.password, {
  contactPerson: formData.fullName || formData.businessName,
  businessName: formData.businessName,
});

// Step 4: Store onboarding data in localStorage (to complete after email verification)
const onboardingData = prepareOnboardingData(docUrls, photoUrls);
localStorage.setItem('pendingSupplierOnboarding', JSON.stringify(onboardingData));

// Step 5: Redirect to confirmation page
router.push('/supplier/confirmation');
} catch (error: any) {
