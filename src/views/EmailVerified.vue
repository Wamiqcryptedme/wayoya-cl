      ...
      <div v-else-if="!errorMessage" class="text-center max-w-md mx-auto animate-fade-in-up">
        ...
        <div class="bg-brand-tealLight p-4 rounded-xl mb-6">
          <p class="text-brand-dark text-sm leading-relaxed">
            We'll send you an email once your account is approved. This usually takes <strong>12-24 hours</strong>.
          </p>
        </div>
...
onMounted(async () => {
  try {
    ...
    // Step 3: Complete onboarding by calling RPC function
    ...
    if (!emailResponse.ok) {
      console.warn('⚠️ Failed to send email, but continuing...');
    } else {
