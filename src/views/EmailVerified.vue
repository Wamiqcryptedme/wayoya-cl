<template>
  <div class="flex flex-col min-h-screen bg-white">
    ...
    <!-- MAIN CONTENT -->
    <main class="flex-1 flex items-center justify-center p-4 md:p-8 py-10">
      ...
      <div v-if="isProcessing" class="text-center max-w-md mx-auto animate-fade-in-up">
        ...
        <h1 class="text-xl font-bold text-brand-dark mb-2">Completing your registration...</h1>
        <p class="text-gray-500 text-sm">Please wait while we set up your account</p>
      </div>
      ...
      <div v-else-if="!errorMessage" class="text-center max-w-md mx-auto animate-fade-in-up">
        ...
        <p class="text-gray-500 text-sm mb-6 leading-relaxed">Your email has been successfully verified.</p>
        
        <!-- Auto-redirect message -->
        <div class="bg-brand-tealLight p-4 rounded-xl mb-6">
          <p class="text-brand-dark text-sm">Redirecting you to your account in <strong>{{ redirectCountdown }}</strong> seconds...</p>
        </div>

        <button 
          @click="redirectNow"
          class="px-6 py-3 bg-brand-teal hover:bg-brand-tealDark text-white font-semibold rounded-xl transition-colors">
      ...
      <div v-else class="text-center max-w-md mx-auto animate-fade-in-up">
        ...
        <button 
          @click="$router.push({ name: 'supplier-signup' })"
          class="px-6 py-3 bg-gray-200 hover:bg-gray-300 text-brand-dark font-semibold rounded-xl transition-colors">
          Return to Signup
        </button>
      </div>
...
onMounted(async () => {
  try {
    // Step 1: Verify the user session exists (Supabase auto-logs in after email verification)
    ...
    // Step 2: Get onboarding data from localStorage
    const storedData = localStorage.getItem('pendingSupplierOnboarding');
    
    if (!storedData) {
      throw new Error('Onboarding data not found. Please complete the signup process again.');
    }

    const onboardingData = JSON.parse(storedData);

    // Step 3: Re-upload documents with actual user ID
    // (Previously uploaded with 'temp' user ID, now we have the real one)
    const userId = session.user.id;
    
    if (onboardingData.verificationDocs) {
      const updatedDocs: Record<string, string> = {};
      
      // Note: In production, you'd need to move the files from temp folder to user folder
      // For now, we'll re-use the URLs but in a real app, implement file migration
      updatedDocs = onboardingData.verificationDocs;
      
      onboardingData.verificationDocs = updatedDocs;
    }

    // Step 4: Complete onboarding by calling RPC function
    ...
    localStorage.removeItem('pendingSupplierOnboarding');

    // Step 6: Success! Show success state
...
const redirectToAppropriateLocation = async () => {
  try {
    const user = await authService.getCurrentUser();
    if (!user) {
      router.push({ name: 'supplier-signup' });
      return;
    }
    ...
    if (!supplier) {
      ...
      router.push({ name: 'supplier-pending' });
      return;
    }
