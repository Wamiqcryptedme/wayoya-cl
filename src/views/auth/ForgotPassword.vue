  ...
  <div class="flex flex-col min-h-screen bg-brand-bg">
    ...
    <main class="flex-1 flex items-center justify-center p-4 md:p-8">
      ...
      <div class="w-full max-w-lg bg-white rounded-xl border border-gray-200 py-6 p-5 md:p-6 md:py-8 my-14 relative">
        ...
        <button @click="$router.back()" class="flex items-center text-sm text-gray-600 hover:text-brand-teal mb-6">
        ...
        <h1 class="text-2xl font-bold text-brand-dark mb-2">Forgot password?</h1>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-5">
          ...
          <!-- Success Message -->
          <div v-if="successMessage" class="p-3 bg-green-100 rounded-xl text-green-700 text-sm flex items-center gap-2">
          ...
          <!-- Error Message -->
          <div v-if="errorMessage" class="p-3 bg-red-50 border border-red-100 rounded-xl text-red-500 text-sm flex items-center gap-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button 
            type="submit" 
            :disabled="isLoading"
            class="w-full py-3 bg-brand-teal text-white font-semibold rounded-xl transition-colors disabled:opacity-50 flex items-center justify-center">
